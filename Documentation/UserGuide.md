...
You need your physical iPhone, Developer Mode and a valid RPPairing file.

1. Open Roam Control and complete the four-page introduction.
2. On **Device Setup**, pair this iPhone through LocalDevVPN, or import an existing RPPairing file.
3. If using on-device pairing, allow Local Network access when iOS asks.
4. If using on-device pairing, open **Settings → Privacy & Security → Developer Mode → Pair with Roam Control**.
5. Enter the six-digit code shown by Roam Control.
6. Choose a connection mode under **Settings → Device → Connection Mode**.
...
Open **Settings → Device → Connection Mode** and choose one of:

- **LocalDevVPN**: use the original on-device LocalDevVPN loopback tunnel.
- **Remote Endpoint**: connect through the configured router endpoint without opening LocalDevVPN. The default is `192.168.31.1:49152`.

Remote Endpoint mode requires an existing valid RPPairing file. The router only forwards encrypted RPPairing/CDTunnel traffic; the pairing file stays on the iPhone.
...
3. Follow the selected connection-mode guidance if it appears.
...
## LocalDevVPN Wi-Fi connection flow
...
## Remote Endpoint connection flow

With Remote Endpoint selected, Roam Control connects directly to the configured router endpoint and does not open or fall back to LocalDevVPN. The default is `192.168.31.1:49152`; the router must forward the subsequent TCP high ports back to this iPhone.

If the endpoint is unavailable, verify the router address, port and network path under **Settings → Device → Connection Mode**.

## LocalDevVPN mobile-data connection flow
...
- Resetting Roam Control restores the connection mode to LocalDevVPN and the default remote endpoint; it does not uninstall or reconfigure LocalDevVPN.
...
### Remote Endpoint is not reachable

Open **Settings → Device → Connection Mode** and verify the endpoint is `192.168.31.1:49152` for the default home-router setup. Confirm that the iPhone can reach the router and that the router forwards TCP `49152-65535` back to this iPhone. Remote Endpoint mode does not fall back to LocalDevVPN automatically.
